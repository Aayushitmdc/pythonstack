## dataos-ctl operate pulsar topics set-persistence
...
```
dataos-ctl operate pulsar topics set-persistence [flags]
```
...
```
  ...
  -a, --bookkeeper-ack-quorum int          Number of acks (guaranteed copies) to wait for each entry
  -e, --bookkeeper-ensemble int            Number of bookies to use for a topic
  -w, --bookkeeper-write-quorum int        How many writes to make of each entry
